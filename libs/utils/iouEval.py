...
class iouEval:
    ...
    def addBatch(self, x, y):   #x=preds, y=targets
        ...
        if (self.ignoreIndex != -1): 
            ...
            x_onehot = x_onehot[:, torch.arange(x_onehot.size(1)) != self.ignoreIndex]
            y_onehot = y_onehot[:, torch.arange(y_onehot.size(1)) != self.ignoreIndex]
        else:
            ignores=0
    ...
    def getIoU(self):
        ...
        iou = (num / den) # "iou per class"
        miou = torch.mean(iou).numpy().tolist() # "iou mean"
        iou = iou.numpy().tolist()
        return {'miou': miou, 'iou_per_class': iou}, miou, iou
