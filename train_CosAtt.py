...
def main():
    ...
    ds = VDS(train=True, transform=opt.test_transforms) #还没进行数据增广
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=400*opt.epochs)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        if ((epoch + 1) % opt.epoch_per_test == 0) or (is_best):
            if not nottrain_Featnet:
                save_checkpoint({
                    'epoch': epoch + 1,
                    'state_dict': feat_net.state_dict()
                }, epoch + 1, is_best, checkpoint=opt.checkpoint_Feat, filename=opt.mode)
            save_checkpoint({
                'epoch': epoch + 1,
...
def train(trainloader, feat_net, task_net, criterion, optimizer, scheduler, epoch, use_cuda):
    ...
    for batch_idx, data in enumerate(trainloader): #循环iter
        ...
        # show_pred(qury_imgs, qury_pred, qury_labels)
        supp_loss = criterion(supp_pred, supp_labels)
        qery_loss = criterion(qury_pred, qury_labels)
        total_loss = supp_loss*supp_weight + qery_loss*qury_weight
        # total_loss = qery_loss*qury_weight*2.0
        total_loss /= B
